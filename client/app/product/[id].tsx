    ...
    const fetchProduct = async () => {
        try {
            ...
            Toast.show({
                ...
                text2: error?.response?.data?.message || 'An error occurred while fetching the product',
            })
        } finally {
            setLoading(false);
        }
        ...
        <View className='relative h-[450px] bg-gray-100 mb-6'>
            ...
            >
                {product.images?.map((image, index) => (
                    <View key={index} >
                        <Image source={{ uri: image }} style={{width: width, height: 450}}  resizeMode='cover' />
                    </View>
                ))}
            </ScrollView>
            {/* Header Actions */}
...
{/* Pagination Dots */}
      <View className='absolute bottom-4 left-0 right-0 flex-row justify-center gap-2'>
        ...
        <View className='px-5'>
            ...
            {/* Price */}
            <Text className='text-2xl font-bold text-primary mb-6'>KSH {product.price.toFixed(2)}</Text>
